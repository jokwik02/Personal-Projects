...
ht* ht_insert(struct ht *ht, char *key, void *value) {
   ...
   //allocate memory for the key here instead
    n->key = calloc(KEY_LEN + 1, sizeof(char)); 
    if (n->key == NULL) {
        perror("calloc:");
        free(n);
        exit(EXIT_FAILURE);
...
char **ht_get_keys(struct ht *ht, int start, int end, int *numb_keys){
    *numb_keys = 0;
    char **keys = malloc(ht->length * sizeof(char*)); 
    
    for (int i = start; i <= end; i++){
        node_t *entry = ht->entries[i];
        //check if the bucket
        while (entry != NULL){
            keys[*numb_keys] = malloc((KEY_LEN + 1) * sizeof(char));
            strncpy(keys[*numb_keys], entry->key, KEY_LEN);
            keys[*numb_keys][KEY_LEN] = '\0';
...
void free_keys(char **keys, int numb_keys) {
    // Free each key
